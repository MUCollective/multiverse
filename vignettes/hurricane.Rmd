...
vignette: >
  ...
  %\VignetteIndexEntry{Are hurricanes with more female names deadlier --- an implementation using multiverse}
...
```{multiverse label = default-m-2, inside = M, echo = FALSE}
df <- df %>%
    mutate(
...
Once we have implemented the analyis model in our multiverse, the corresponding step will be applied to each analysis path. To interpret the results, we first estimate a prediction interval corresponding to each analysis path. 
