...
#Or, we could get the code for a different single universe analysis by providing a list of parameter assignments (options which each parameter should take) to the `execute_universe()` function and execute it:
...
```{r}
p <- expand(M) %>%
  extract_variables(summary_FiscConsComp) %>%
...
p <- expand(M) %>%
  ...
  unnest( cols = c(summary_SocConsComp) ) %>%
  mutate( term = recode( term, 
                 "RelationshipStatusSingle" = "Single",
                 "Fertilitylow:RelationshipStatusSingle" = "Single:Fertility_low"
  ) ) %>%
