...
a_list.ref = parse_multiverse(M)@multiverse_table$parameter_assignment

list(
  lapply(c("mc_option1", "mc_option2", "mc_option3"), function(x) list(menstrual_calculation = x)),
  lapply(c("cer_option1", "cer_option2"), function(x) list(certainty = x))
)
...
identical(a_list.ref, transpose(a_df))
```

```{r}
M.parsed = parse_multiverse(M)

M.evaluated = parse_multiverse(M) %>%
  execute_multiverse()
```
