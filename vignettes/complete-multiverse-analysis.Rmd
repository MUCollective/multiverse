...
<span style="colour:"red"> __this does not work__ </span>
...
M@multiverse_table$results[[1]]$df
```
...
Inspecting the `multiverse_table`, we can see that all the possible combinations of the parameter options have been identified.
...
*Note:* since we did not store the manipulations as a variable, due to R's copy-on-modify behavior, we cannot access this from the `multiverse_table` like we did before.
...
M@multiverse_table$results[[2]]$df
```
