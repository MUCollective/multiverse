...
1. `%when%`: when declaring a branch, we can use this operator to specify the condition as `A %when% B`
2. `branch_assert`: the conditional $A | B$ is also referred to as the connective $A \implies B$. This has the meaning "if A is true, then B is also true" and is an abbreviation for $\neg A | B$
...
Within the branch assert function, the user needs to specify the classical logic specification $\neg A | B$:
...
Now, we've implemented the complete multiverse example #2 from Steegen et al.'s paper
