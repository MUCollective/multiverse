...
```{r}
M$current_parameter_assignment
```
...
```{r generate_code, eval = FALSE}
get_code(M, list(menstrual_calculation = "mc_option2"))%>%
  rlang::eval_tidy() %>%
...
Finally, we can get the code for the defualt single universe analysis from this multiverse and execute it:

```{r, eval = FALSE}
get_code(M) %>%
  rlang::eval_tidy() %>%
  select(ComputedCycleLength, NextMenstrualOnset, CycleDay, Relationship, Fertility)
```
...
get_code(M, universe_parameter_assignment) %>%
  rlang::eval_tidy() %>%
  select(ComputedCycleLength, NextMenstrualOnset, CycleDay, Relationship, Fertility)
```
