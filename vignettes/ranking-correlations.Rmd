...
df <- vis_correlation2 %>% unite("visandsign", vis:rdirection) %>% 
        rename(r = rbase, mr = rv, correct = gotItRight) %>% 
        mutate(dr = round(abs(mr - r) * 100)/100, participant = as.factor(participant)) %>% 
        select(-jnd) %>% filter(participant %in% c("hrqfen7i", 
        "hs1tlxtw", "hskjvwol", "hsyws9iw", "hsi8loaf"))

derive_jnd <- function(df) {
        psy_data <- df %>% group_by(participant, r, approach, 
            visandsign, dr) %>% summarise(k = sum(as.integer(correct)), 
            n = n(), group = "drop")
        fit <- quickpsy(psy_data, dr, k, n, grouping = .(participant, 
            r, approach, visandsign), bootstrap = "none", guess = 0.5, 
            lapses = TRUE, log = TRUE, prob = 0.75, parini = c(0.2, 
                0.5, 0.01))
        df <- fit$thresholds %>% group_by(participant, r, approach, 
            visandsign) %>% rename(jnd = thre) %>% full_join(df, 
            by = c("participant", "r", "approach", "visandsign")) %>% 
            ungroup() %>% drop_na()
        return(df)
    }
    df <- derive_jnd(df)

df <- df %>% group_by(visandsign) %>% mutate(p_chance_cutoff = (mean(jnd > 
        0.45) > 0.2)) %>% group_by(visandsign, r, approach) %>% 
        mutate(mad_cutoff = FALSE) %>% ungroup() %>% mutate(exclude = p_chance_cutoff | 
        mad_cutoff) %>% filter(!exclude)

df <- df %>% group_by(visandsign, r, approach) %>% mutate(mean_jnd_per_approach = mean(jnd)) %>% 
        group_by(visandsign, r) %>% mutate(mean_jnd_within_r = mean(mean_jnd_per_approach), 
        r = r + ifelse(approach == "above", 0.5, -0.5) * mean_jnd_within_r) %>% 
        ungroup() %>% mutate(approach = ordered(approach, levels = c("above", 
        "below")), approach_value = if_else(approach == "above", 
        -1, 1))

m <- glm(jnd ~ r * visandsign * approach_value, data = df, family = gaussian(link = "log"))
```


```{r}
output <- m %>% 
  emmeans("visandsign", type = "response") %>% 
  summary()
```
...
M <- multiverse()
add_and_parse_code(M, parse(text = c("{", pasted, "}"), keep.source = FALSE)[[1]])
...
```{r}
expand(M)$.code[[1]]$postprocessing
```
