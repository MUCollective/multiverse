...
expand(M) %>%
  ...
  group_by(term) %>%
  mutate(
    min = map2_dbl(estimate, std.error, ~ qnorm(0.001, .x, .y)),
    min = min(min),
    max = map2_dbl(estimate, std.error, ~ qnorm(0.999, .x, .y)),
    max = max(max),
    cdf.x = map2(min, max, ~ seq(.x, .y, length.out = 101)),
    cdf.y = pmap(list(estimate, std.error, cdf.x), ~ pnorm(..3, ..1, ..2))
  ) %>%
  select(-.code, -.results) %>%
  group_by(.universe) %>%
  nest(results = term:cdf.y) %>%
