...
transform_fun <- branch(data_trans,
    "log-transformed" ~ log,
    "un-transformed" ~ identity
  )
...
df <- data.userlogs.raw  %>%
    mutate( duration = transform_fun(duration) ) %>% 
...
Consider the modeling step from `vignette("example-hurricane")`. Here, one of the decisions that we face is the choice of regression model to use (gaussian or poisson). As described in the document, this requires two different local changes in the code: 
...
## Example 8: Declaring continuous variables in `branch`
...
## Example 9: Nested branch statements and asymmetric trees

Consider the following hypothetical dataset where the analyst wants to predict the variable `score` based on the measures `pretest1`, `pretest2`, `reaction_time`, `HR` (heart rate) and `fatigue`.
...
They come up with the following regression models—normal and binomial, and in the normal model they include a decision to include or not include an interaction term for two of the variables. While one way of defining such a multiverse is through the use of the conditional operator (see `vignette("conditions")`, this can also be defined using nested branch statements:

```{multiverse h-11, inside = M, echo = FALSE}
fit = branch(model, 
         "normal" ~ glm(log(score) ~ pretest1 + pretest2 + fatigue +
                        branch(interaction, 
                               "yes" ~ reaction_time*HR,
                               "no" ~ reaction_time + HR) + 
                        fatigue, data = df, family = "gaussian"),
         "gamma" ~ glm(score ~ pretest1 + pretest2 + reaction_time + HR + 
                          fatigue, data = df, family = "gamma")
      )
```


````
```{multiverse default-m-11, inside = M}`r ''`
fit = branch(model, 
...
The resultant multiverse should consist of three distinct analysis paths:
...
However, because of how we currently process the multiverse code, if nested branch statements result in asymmetric trees such as the one above, we do not get the correct output. Until we fix this issue, we resort to providing the user with a warning whenever such potentially asymmetric trees are detected:

```{r}
expand(M)
```
The solution, which is essentially a workaround is to remove duplicates, *upon verification* at the post-processing stage (and we can then safely ignore the warning).
