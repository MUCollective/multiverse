...
author: "Yvonne Jansen and Abhraneel Sarma"
date: "`r Sys.Date()`"
...
In this vignette, we will recreate the multiverse analysis, [Frequentist](https://explorablemultiverse.github.io/examples/frequentist/), performed by Dragicevic et al. in [Increasing the transparency of research papers with explorable multiverse analyses](https://hal.inria.fr/hal-01976951/document) using the package. The re-analysis is of a study which shows that moving 3D data visualizations to the physical world can improve users' efficiency at information retrieval tasks.
...
We will use the multiverse code block to enter code into the multiverse. See the {\link{multiverse-code-block} for more details.
...
```{multiverse, label = default-m-5, inside = M}
df.diffs
```
...
```{r}
df.results.diff <- multiverse_table(M) %>%
