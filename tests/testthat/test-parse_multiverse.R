...
# parsing branches with multiple assignments --------------------------------------------
test_that("branch should support multiple assignments within a code block", {
  M <- multiverse()
  
  inside(M, branch(multiple_assignments, 
        "one" ~ {
          one <- 1
        },
        "two" ~ {
  ...
  inside(M2, branch(multiple_assignments,
                   "three" ~ {
                     one <- 1
                     two <- 2
                     three <- 3
  ...
  expect_equal(unname(extract_variables(M, "one")$one), c(1, 1, 1))
  expect_equal(unname(extract_variables(M, "two")$two), c(NA, 2, 2))
  expect_equal(unname(extract_variables(M, "three")$three), c(NA, NA, 3))
  
  execute_multiverse(M2)
  expect_equal(unname(extract_variables(M2, "one")$one), c(1, 1, 1))
  expect_equal(unname(extract_variables(M2, "two")$two), c(2, 2, NA))
  expect_equal(unname(extract_variables(M2, "three")$three), c(3, NA, NA))
})
